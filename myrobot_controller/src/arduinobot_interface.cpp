#include "myrobot_controller/aduinobot_inteface.hpp"
                                                           ...
                                                           const rclcpp::Duration &period)
{
  ...
  msg.append(",");
  int xxx = static_cast<int>(((position_commands_.at(3) ) * 180) / M_PI);
  msg.append("x");
  msg.append(std::to_string(xxx));
  msg.append(",");
  int yyy = static_cast<int>(((position_commands_.at(4) ) * 180) / M_PI);
  msg.append("y");
  msg.append(std::to_string(yyy));
  msg.append(",");
  int gripper = static_cast<int>(((position_commands_.at(5)) * 180) / (M_PI));
  msg.append("g");
